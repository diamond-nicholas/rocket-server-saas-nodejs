...
const { extractCookies, shapeFlags } = require("../utils/extractCookies");
const stripeService = require('../../src/services/stripe.service');

jest.mock('../../src/services/stripe.service');
stripeService.createCustomer.mockResolvedValue({id: faker.random.uuid()});
...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    beforeEach(() => {
      ...
      stripeService.createCustomer.mockResolvedValue({id: stripeId});
    });
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      ...
      expect(res.body.user).not.toHaveProperty('stripeId');
      expect(res.body.user).toEqual({ id: expect.anything(), name: newUser.name, email: newUser.email, role: 'user', subscription: {subscriptionType: 'free'} });

      const dbUser = await User.findById(res.body.user.id);
      expect(dbUser).toBeDefined();
      expect(dbUser.password).not.toBe(newUser.password);
      expect(dbUser).toMatchObject({ name: newUser.name, email: newUser.email, role: 'user', stripeId: stripeId, subscription: {subscriptionType: 'free'} });

      expect(res.body.token).toEqual({
        token: expect.anything(), expires: expect.anything(),
      });
    });
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      await request(app).post('/v1/auth/logout').set('Cookie', ['refreshToken='+refreshToken]).send().expect(httpStatus.NO_CONTENT);
    ...
    test('should return 404 error if refresh token is not found in the database', async () => {
      ...
      await request(app).post('/v1/auth/logout').set('Cookie', ['refreshToken='+refreshToken]).send().expect(httpStatus.NOT_FOUND);
    });
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      const res = await request(app).post('/v1/auth/refresh-tokens').set('Cookie', ['refreshToken='+refreshToken]).send().expect(httpStatus.OK);
      ...
      expect(cookies.refreshToken.flags).toBeDefined();
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      ...
      await request(app).post('/v1/auth/refresh-tokens').set('Cookie', ['refreshToken='+refreshToken]).send().expect(httpStatus.UNAUTHORIZED);
    });
