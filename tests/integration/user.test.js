...
describe('User routes', () => {
  describe('POST /v1/users', () => {
    ...
    test('should return 201 and successfully create new user if data is ok', async () => {
      ...
      expect(res.body).not.toHaveProperty('stripeId');
      expect(res.body).toEqual({ id: expect.anything(), name: newUser.name, email: newUser.email, role: newUser.role, subscription: {subscriptionType: 'free'} });

      const dbUser = await User.findById(res.body.id);
      expect(dbUser).toBeDefined();
      expect(dbUser.password).not.toBe(newUser.password);
      expect(dbUser).toMatchObject({ name: newUser.name, email: newUser.email, role: newUser.role, stripeId: stripeId, subscription: {subscriptionType: 'free'} });
    });
  ...
  describe('GET /v1/users', () => {
    test('should return 200 and apply the default query options', async () => {
      await insertUsers([userOne, userTwo, userThree, admin]);
      ...
      expect(res.body.results[0]).toEqual({
        ...
        role: userOne.role,
        subscription: {
          subscriptionType: userOne.subscription.subscriptionType,
        },
      });
    });
    ...
    test('should correctly sort the returned array if ascending sort param is specified', async () => {
      ...
      expect(res.body.results[3].id).toBe(userThree._id.toHexString());
    });
